<template>
  <div class="app-container">
    ...
    <es-goods v-for="item in goodslist" :key="item.id" :id="item.id" :thumb="item.goods_img" :title="item.goods_name"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getCount(val) {
        const findResult = this.goodslist.find(x => x.id === val.id)
        if(findResult) {
          findResult.goods_count = val.value
        }
      }
    },
    computed: {
